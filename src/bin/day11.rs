            ...
            for col in row.1.iter().enumerate() {
                ...
                for i in 0..3 {
                    for j in 0..3 {
                        ...
                        let r: i32 = row.0 as i32 + i - 1;
                        let c: i32 = col.0 as i32 + j - 1;

                        if r >= 0 && c >= 0 && (r as usize) < grid.len()  && (c as usize) < row.1.len() && grid[r as usize][c as usize] == '#' as u8 {
                            occ += 1;
                        }
                    }
                }
                ...
                new_row.push(match *col.1 as char {
                    'L' if occ == 0 => { changes += 1; '#' as u8 },
                    '#' if occ >= 4 => { changes += 1; 'L' as u8 },
                    x => x as u8,
                });
            }
                ...
                for dr in &[-1, 0, 1] {
                    for dc in &[-1, 0, 1] {
                        ...
                        loop {
                            ...
                            if r >= 0 && c >= 0 && (r as usize) < grid.len() && (c as usize) < row.1.len() {
