...
fn solve1(nums: &[i64]) -> Result<i64, Box<dyn Error>> {
...
fn solve2(nums: &[i64]) -> Result<i64, Box<dyn Error>> {
    let mut set: HashSet<i64> = HashSet::new();
    set.extend(nums);
